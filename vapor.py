...
def init(counter):
    ...
    if hasattr(os, "sched_getaffinity"):
        ...
        # affinity = os.sched_getaffinity(0)
    logging.info(
        f"\tWorker: init. rank={rank} pid={pid} ID={pidmap[pid]} affinity={affinity}"
    )
...
def dowork(X, inode, num_channels):
    ...
    den0, u_para0, T_perp0, T_para0, _, _ = xgcexp.f0_diag(
        f0_inode1=inode % nnodes, ndata=num_channels, isp=1, f0_f=f0_f, progress=False
    )
    ...
    f0_f *= mx[:, np.newaxis, np.newaxis] - mn[:, np.newaxis, np.newaxis]
    f0_f += mn[:, np.newaxis, np.newaxis]
    den1, u_para1, T_perp1, T_para1, _, _ = xgcexp.f0_diag(
        f0_inode1=inode % nnodes, ndata=num_channels, isp=1, f0_f=f0_f, progress=False
    )
...
def physics_loss(data, lb, data_recon, progress=False):
    ...
    den_err = torch.tensor(0.0).to(device)
    u_para_err = torch.tensor(0.0).to(device)
    T_perp_err = torch.tensor(0.0).to(device)
    T_para_err = torch.tensor(0.0).to(device)
    ...
    for i in tqdm(range(batch_size), disable=not progress):
        ...
        # f0_f = data[i].cpu().data.numpy()
        f0_f = torch.from_numpy(Z0[inode : inode + num_channels, :nvp0, :nvp1]).to(
            device
        )
        # f0_f *= (mx[:,np.newaxis,np.newaxis]-mn[:,np.newaxis,np.newaxis])
        # f0_f += mn[:,np.newaxis,np.newaxis]
        den0, u_para0, T_perp0, T_para0, _, _ = xgcexp.f0_diag_torch(
            f0_inode1=inode % nnodes,
            ndata=num_channels,
            isp=1,
            f0_f=f0_f,
            progress=False,
        ...
        f0_f2 = f0_f1 * torch.from_numpy(
            mx[:, np.newaxis, np.newaxis] - mn[:, np.newaxis, np.newaxis]
        ).to(device)
        f0_f = f0_f2 + torch.from_numpy(mn[:, np.newaxis, np.newaxis]).to(device)
        den1, u_para1, T_perp1, T_para1, _, _ = xgcexp.f0_diag_torch(
            f0_inode1=inode % nnodes,
            ndata=num_channels,
            isp=1,
            f0_f=f0_f,
...
# %%
def read_f0_nodes(
    istep, inodes, expdir=None, iphi=None, nextnode_arr=None, rescale=None
):
    """
    Read XGC f0 data
    """
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(lx), nbatch):
            ...
            if modelname in ("vae", "cvae"):
                ...
                valid_reconstructions = valid_reconstructions.view(
                    -1, model.nc, model.ny, model.nx
                )
...
def estimate_error(
    ...
    if fname is not None:
        ...
        dat = torch.cat(
            (
                torch.tensor(Zif[idx, np.newaxis, :, :]),
                torch.tensor(Xbar[idx, np.newaxis, :, :]),
            )
        )
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        encodings = torch.zeros(encoding_indices.shape[0], self._num_embeddings).to(
            device
        )
...
class ResidualLinearStack(nn.Module):
    def __init__(
        ...
        self._layers = nn.ModuleList(
            [
                ResidualLinear(in_channels, num_hiddens, num_residual_hiddens)
                for _ in range(self._num_residual_layers)
            ]
        )
...
class VAE(nn.Module):
    ...
    def loss_function(self, recon_x, x, mu, logvar):
        BCE = F.binary_cross_entropy(
            recon_x, x.view(-1, self.nx * self.ny), reduction="sum"
        )
...
class AE(nn.Module):
    def __init__(
        ...
        self._encoder = nn.Sequential()
        for i, (in_size, out_size) in enumerate(
            zip(encoder_layer_sizes[:-1], encoder_layer_sizes[1:])
        ):
            if i == 0:
                in_size += self.ncond
        ...
        for i, (in_size, out_size) in enumerate(
            zip(decoder_layer_sizes[:-1], decoder_layer_sizes[1:])
        ):
            if i == 0:
                in_size += self.ncond
...
class SpectralConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if False:
            ...
            m = x_ft.shape[2] // 2
            out_ft[:, :, m - self.modes1 : m, -self.modes2 :] = compl_mul2d(
                x_ft[:, :, m - self.modes1 : m, -self.modes2 :], self.weights1
            )
            out_ft[:, :, m : m + self.modes1, -self.modes2 :] = compl_mul2d(
                x_ft[:, :, m : m + self.modes1, -self.modes2 :], self.weights2
            )
...
def main():
    ...
    parser.add_argument(
        ...
        help="num_training_updates (default: %(default)s)",
        type=int,
        default=10_000,
    )
    parser.add_argument(
        "-E",
        "--embedding_dim",
        help="embedding_dim (default: %(default)s)",
        type=int,
        default=16,
    )
    parser.add_argument(
        "-e",
        "--num_embeddings",
        help="num_embeddings (default: %(default)s)",
        type=int,
        default=512,
    )
    parser.add_argument(
        "-H",
        "--num_hiddens",
        help="num_hidden (default: %(default)s)",
        type=int,
        default=128,
    )
    parser.add_argument(
        "-R",
        "--num_residual_hiddens",
        help="num_residual_hiddens (default: %(default)s)",
        type=int,
        default=32,
    )
    parser.add_argument(
        "-L",
        "--num_residual_layers",
        help="num_residual_layers (default: %(default)s)",
        type=int,
        default=2,
    )
    parser.add_argument(
        "-C", "--num_channels", help="num_channels", type=int, default=1
    )
    parser.add_argument(
        "-B",
        "--batch_size",
        help="batch_size (default: %(default)s)",
        type=int,
        default=32,
    )
    parser.add_argument(
        "-d",
        "--device_id",
        help="device_id (default: %(default)s)",
        type=int,
        default=0,
    )
    parser.add_argument(
        "--wdir", help="working directory (default: current)", default=os.getcwd()
    )
    parser.add_argument(
        "--datadir",
        help="data directory (default: %(default)s)",
        default="d3d_coarse_v2",
    )
    ...
    parser.add_argument(
        "--average_interval", help="average_interval (default: %(default)s)", type=int
    )
    parser.add_argument(
        "--log_interval",
        help="log_interval (default: %(default)s)",
        type=int,
        default=1_000,
    )
    parser.add_argument(
        "--checkpoint_interval",
        help="checkpoint_interval (default: %(default)s)",
        type=int,
        default=10_000,
    )
    ...
    parser.add_argument(
        "--noise", help="noise value in (0,1)", type=float, default=None
    )
    parser.add_argument("--resampling", help="resampling", action="store_true")
    parser.add_argument(
        "--resampling_interval", help="resampling_interval", type=int, default=None
    )
    parser.add_argument("--overwrite", help="overwrite", action="store_true")
    parser.add_argument(
        "--learning_rate", "--lr", help="learning_rate", type=float, default=1e-3
    )
    ...
    parser.add_argument(
        "--milestones", help="scheduler milestones", nargs="*", type=int, default=None
    )
    parser.add_argument(
        "--stepsize", help="scheduler step size", type=int, default=50_000
    )
    ...
    group1.add_argument(
        "--physicsloss_interval",
        help="physicsloss_interval (default: %(default)s)",
        type=int,
        default=1,
    )
    ...
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "--vae", help="vae model", action="store_const", dest="model", const="vae"
    )
    group.add_argument(
        "--cvae", help="cvae model", action="store_const", dest="model", const="cvae"
    )
    group.add_argument(
        "--vqvae", help="vqvae model", action="store_const", dest="model", const="vqvae"
    )
    group.add_argument(
        "--cvqvae",
        help="cvqvae model",
    ...
    group.add_argument(
        "--gan", help="gan model", action="store_const", dest="model", const="gan"
    )
    group.add_argument(
        "--fno", help="fno model", action="store_const", dest="model", const="fno"
    )
    group.add_argument(
        "--ae", help="ae model", action="store_const", dest="model", const="ae"
    )
    group.add_argument(
        "--cae", help="cae model", action="store_const", dest="model", const="cae"
    )
    group.add_argument(
        "--ae-vqvae",
        help="ae-vqvae model",
        ...
        for istep in timesteps:
            ...
            if args.surfid is not None:
                ...
                _out = read_f0(
                    ...
                    inode=args.inode,
                    nnodes=args.nnodes,
                    randomread=args.randomread,
                    nchunk=num_channels,
                    fieldline=args.fieldline,
                )
                f0_data_list.append(_out)
                if args.hr:
                    assert args.hr_datadir is not None
    ...
    if args.dataset == "nstx":
        Z0, Zif, zmu, zsig, zmin, zmax, zlb = read_nstx(
            args.datadir, args.offset, args.nframes, gaussian=args.gaussian
        )
        Xif = Zif
    ...
    if args.model == "fno":
        ...
        full_loader = torch.utils.data.DataLoader(
            full_data, batch_size=1, shuffle=False
        )
        ...
        for ep in range(istart, istart + args.num_training_updates):
            ...
            with torch.no_grad():
                ...
                for x, y in test_loader:
                    ...
                    abs_err = max(
                        abs_err, torch.max(nn.L1Loss(reduction="none")(y, out)).item()
                    )
                    test_err += myloss(
            ...
            if (ep % args.checkpoint_interval == 0) and (rank == 0):
                ...
                for x, y in full_loader:
                    ...
                    abs_err = max(
                        abs_err,
                        torch.max(nn.L1Loss(reduction="none")(y.squeeze(), out)).item(),
                    )
    ...
    training_data = torch.utils.data.TensorDataset(
        torch.tensor(lx), torch.tensor(ly), torch.tensor(lh)
    )
    ...
    validation_loader = DataLoader(
        validation_data, batch_size=batch_size, shuffle=True, pin_memory=True
    )
    ...
    def setup_da():
        ...
        global da
        da = np.zeros(
            (len(zlb), 2), dtype=np.float32
        )  ## list of distance and angle pair
    ...
    if args.model in ("vqvae", "cvqvae"):
        ...
        model = Model(
            ...
            learndiff=args.learndiff,
            shaconv=args.shaconv,
            grid=grid,
            conditional=args.conditional,
            decoder_padding=padding,
            da_conditional=da_conditional,
    ...
    if args.model == "gan":
        ...
        optimizer_D = optim.AdamW(
            discriminator.parameters(), lr=learning_rate, amsgrad=False
        )

    if args.model == "ae":
        log(
            "AE model: input_dim: %d, embedding_dim= %d"
            % (num_channels * ny * nx, args.embedding_dim)
        )
        model = AE(
            input_dim=num_channels * ny * nx,
            embedding_dim=args.embedding_dim,
    ...
    if args.model == "cae":
        setup_da()
        log(
            "CAE model: input_dim: %d, embedding_dim= %d"
            % (num_channels * ny * nx, args.embedding_dim)
        )
        model = AE(
            input_dim=num_channels * ny * nx,
    ...
    if args.milestones is not None:
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, step_size=args.stepsize, gamma=0.1
        )
    ...
    executor = ProcessPoolExecutor(
        max_workers=nworkers, initializer=init, initargs=(counter,)
    )
    num_training_updates = args.num_training_updates
    resampling_interval = (
        len(lx) // batch_size * 10
        if args.resampling_interval is None
        else args.resampling_interval
    )
    logging.info(
        f"Rsampling, resampling interval: {args.resampling} {resampling_interval}"
    )
    ...
    for i in xrange(istart, istart + num_training_updates):
        ...
        if args.model in ("vqvae", "cvqvae"):
            ...
            if args.physicsloss and (i % args.physicsloss_interval == 0):
                ...
                den_err, u_para_err, T_perp_err, T_para_err = physics_loss(
                    hr_data, lb, data_recon
                )
        ...
        if args.model in ("vae", "cvae"):
            ...
            recon_error = (
                F.mse_loss(recon_batch, hr_data.view(-1, nx * ny)) / hr_data_variance
            )
        ...
        if args.resampling and (i % resampling_interval == 0):
            ...
            err = np.array(
                [
                    max(err_list[i : i + num_channels])
                    for i in xrange(0, len(err_list), num_channels)
                ]
            )
            ...
            training_data = torch.utils.data.TensorDataset(
                torch.tensor(lxx), torch.tensor(lyy), torch.tensor(lhh)
            )
            training_loader = DataLoader(
                training_data, batch_size=batch_size, shuffle=True, pin_memory=True
            )
        ...
        if i % args.log_interval == 0:
            ...
            logging.info(
                f"{i} Avg: {np.mean(train_res_recon_error[-args.log_interval:]):g} {np.mean(train_res_perplexity[-args.log_interval:]):g} {np.mean(train_res_physics_error[-args.log_interval:]):g}"
            )
            if args.model in ("vqvae", "cvqvae"):
                logging.info(
                    f"{i} Loss: {recon_error.item():g} {vq_loss.data.item():g} {perplexity.item():g} {physics_error:g} {dloss:g} {feature_loss.item():g} {len(training_loader.dataset)} {len(data)}"
                )
            ...
            if args.model == "gan":
                logging.info(
                    f"{i} Loss: {recon_error.item():g} {vq_loss.data.item():g} {perplexity.item():g} {physics_error:g} {dloss:g} {len(training_loader.dataset)} {len(data)}"
                )
            ...
            rmse_list, abserr_list = estimate_error(
                model,
                Xif,
                Zif,
                zmin,
            ...
            logging.info(
                f'{i} Error: {np.max(rmse_list):g} {np.max(abserr_list):g} Next LR: {optimizer.param_groups[0]["lr"]:g}'
            )
    ...
    with torch.no_grad():
        ...
        if args.model in ("vqvae", "cvqvae"):
            ...
            logging.info(
                "Quantized: %s" % (valid_quantize.detach().cpu().numpy().shape,)
            )
            logging.info(
                "Reconstructed: %s"
                % (valid_reconstructions.detach().cpu().numpy().shape,)
            )
            logging.info(
                "compression ratio: %.2fx"
                % (
                    valid_originals.cpu().numpy().size
                    / vq_encoded.detach().cpu().numpy().size
                )
            )
            logging.info(
                "compression ratio: %.2fx"
                % (
                    valid_originals.cpu().numpy().size
                    / valid_quantize.detach().cpu().numpy().size
                )
            )
        ...
        info(
            "RMSE error: %g %g %g"
            % (np.min(rmse_list), np.mean(rmse_list), np.max(rmse_list))
        )
        info(
            "ABS error: %g %g %g"
        ...
        info(
            "All (total, MB, ratio): %d %g %g"
            % (num_params, num_params * 4 / 1024 / 1024, num_params / nx / ny)
        )
